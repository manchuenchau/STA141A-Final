---
title: "Analysis and Prediction of the Trend of the Death in COVID-19 Pandemic of USA"
...
## import data from WHO website
global_covid <- read_csv("https://covid19.who.int/WHO-COVID-19-global-data.csv")
## import data from the local computer
...
Extract US data from the global data set and show the summary table of the US data:
...
```
From the above summary, new cases and new deaths of US data contain no negative values. In other words, the data the US government provided to WHO is correct and she did not need to adjust it. Date range is from Jan 20, 2020 to Mar 04, 2022; that is up to date. The number of rows is 792. The means of new cases and new deaths for USA are much higher than the means of the global data. That means the situation in USA is very serious.
...
However, we have to trust the data from WHO which have no sampling problems, measurement errors or data entry errors (Which covid-19 data can you trust? 2021). Thus, those outliers will not be removed from the data set.
...
- Cumulative Cases and Cumulative Deaths are dependent with each other.
...
```
From the above result, the smallest value of BIC is -228.7251 at the first row. The first row of the Selection Algorithm exhaustive table shows that only New_cases has an asterisk. In other words, only New_cases should be selected for the model.
...
Normal Q-Q: If the residuals points follow the dashed straight line, the residuals are normally distributed. Nevertheless, so many data which are less than -1.5 or more than 1 appear not to be normally distributed. However, the sample size is very large. This is no problem. The residuals can be supposed to be normally distributed.
...
Critical values: p-value = 0.01311
...
The regression model:
  ...
  $\sqrt{\hat{Y}}$ = 30.9736 + 225.1097$X}$ - 201.1008$X^2$ + 119.7143$X^3$ - 79.6982$X^4$
...
y <- 30.9736 + 225.1097*x - 201.1008*x^2 + 119.7143*x^3 - 79.6982*x^4
plot(x, y, xlab = "x\nFigure 9")


```
Noted that the figure 9 shows that the equation: 30.9736 + 225.1097$X}$ - 201.1008$X^2$ + 119.7143$X^3$ - 79.6982$X^4$ is concave downward (downward sloping when x > 0). It implies that the the number of deaths will decrease as the number of new cases increases. Nevertheless, we have to plot the transformed data of the model either:
...
```
The figure 10 shows us that the number of deaths does not decrease significantly when the number of new cases increases though the number of deaths appears to be downward sloping when number of new cases is over 700000. Furthermore, the figure 11 shows us that the trend of the death rate is upward sloping after Jan, 2022. In other words, increase in new cases also leads to proportional increase in new deaths. That does not mean the trend of deaths will grow up because so many new cases have not been reported. In any case, the number of deaths still keeps at a lower level (i.e. the rate is less than 0.05).
